  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    #stock-menu {{ position: fixed; z-index: 9999; background: #1e293b; border: 1px solid #334155;
      border-radius: 10px; box-shadow: 0 8px 24px rgba(0,0,0,.5); padding: 6px 0; min-width: 200px;
      display: none; }}
    #stock-menu.show {{ display: block; }}
    #stock-menu .menu-header {{ padding: 7px 14px 5px; font-size: 0.72rem; color: #64748b;
...
(function () {{
  ...
  function renderTable(cmp, streaks, isMulti) {{
    ...
    tbody.innerHTML = allRows.map(row => {{
      ...
      const wcCell = `<td class="${{row.isAdded?'':cls(row.weightDiff)}}" data-sort="${{row.weightDiff??0}}">${{row.isAdded ? '—' : fmtWDiff(row.weightDiff)}}</td>`;
